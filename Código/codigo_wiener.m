...
%--------------------------------------------------------------
[y, Fs] = audioread('Interferência_Ruído-Ambiente.wav');      %Utilizar um sinal já com o ruído embutido
 %noise = audioread('ruído_branco.wav');
 %y = y + 0.3*noise;
...
audiowrite('Wiener_Ruído-Ambiente.wav',filtered_speech_wiener,Fs);
...
colormap('parula');
colorbar;
title('Espectrograma: sinal + ruído');
xlabel('Tempo (s)');
